...
namespace frou01.util
{
    public class terrainDigger : MonoBehaviour
    {
        ...
        public void DigTerrain()
        {
            ...
            int pickSquarex = 1 + (int)(range / terrainData.size.x * (heights.GetLength(0) - 1));
            int pickSquarez = 1 + (int)(range / terrainData.size.z * (heights.GetLength(1) - 1));
            ...
            for (float step = start; step <= end; step += range / 2)
            {
                ...
                Vector3 onTerrainPos = path.EvaluatePositionAtUnit(step, PositionUnits.Distance);
                onTerrainPos -= target.transform.position;
                int onPathX = (int)Mathf.Floor(onTerrainPos.x / terrainData.size.x * (heights.GetLength(0) - 1));
                int onPathZ = (int)Mathf.Floor(onTerrainPos.z / terrainData.size.z * (heights.GetLength(1) - 1));
                //Debug.Log(step);
                ...
                for (int xID = onPathX - pickSquarex; xID <= onPathX + pickSquarex && xID < heights.GetLength(0); xID++)
                {
                    for (int zID = onPathZ - pickSquarez; zID <= onPathZ + pickSquarex && zID < heights.GetLength(1); zID++)
                    {
                        ...
                        if (path.FromPathNativeUnits(closestUnit, PositionUnits.Distance) < start
                            ||
                           path.FromPathNativeUnits(closestUnit, PositionUnits.Distance) > end) continue;
